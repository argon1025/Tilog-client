...
export default function UserPinnedRepoCommponent({ username }) {
    ...
    return(
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 justify-center items-center">
          {/* Card */}
          {!userpinnedrepo ? <>fetching...</> 
          : userpinnedrepo.map((repo)=>(
          <div key={repo.id} className="flex flex-col w-full h-44 border border-gray-200 rounded-3xl bg-white filter drop-shadow-2xl">
          {/* Card title */}
          ...
          <div className="flex flex-row mx-5">
            ...
            <div className="flex items-center justify-center w-9 h-9 border border-gray-400 rounded-xl mr-2">
              <TechIconLoader iconName={repo.primaryLanguage.name} color="#393939" />
            </div>
            {/* icon example-2 */}
            {/* <div className="flex items-center justify-center w-9 h-9 border border-gray-400 rounded-xl  mr-2">
          ...
          <div className="flex mx-5">
            ...
            <div className="flex items-center ml-auto">
              ...
              <p className="text-gray-700 text-xs">{repo.stargazers.totalCount}</p>
            </div>
          </div>
        </div>
          ))}
