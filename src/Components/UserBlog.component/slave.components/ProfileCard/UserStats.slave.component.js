    ...
    return(
          <div className="flex lg:flex-row items-center lg:items-start flex-col max-w-5xl w-full bg-white mt-10 p-10 rounded-lg border border-gray-00">
            <img
              ...
              src={!userInfo ?<>fetching..</>: userInfo.proFileImageUrl}
              alt="profile"
            />
            <div className="m-5">
              {/* Name, message */}
              <p className="text-xl text-gray-700 font-bold">
                  {!userStats ?<>fetching..</>: <>{`${userStats.name}(${username})`}</>}
              </p>
              ...
              <div className="flex mt-3 items-center">
                <IconContext.Provider value={{ className: "w-4 h-4" }}>
                  <GoStar />
                  <p className="text-sm text-gray-700 font-bold mr-1">{!userStats ?<>fetching..</>:userStats.totalStars}</p>
                  <p className="text-sm text-gray-400">stars</p>
                </IconContext.Provider>
                <IconContext.Provider value={{ className: "ml-2 w-4 h-4" }}>
                  <GoGitPullRequest />
                  <p className="text-sm text-gray-700 font-bold mr-1">{!userStats ?<>fetching..</>:userStats.totalPRs}</p>
                  <p className="text-sm text-gray-400">PRs</p>
                </IconContext.Provider>
                <IconContext.Provider value={{ className: "ml-2 w-4 h-4" }}>
                  <GoGitCommit />
                  <p className="text-sm text-gray-700 font-bold mr-1">{!userStats ?<>fetching..</>:userStats.totalCommits}</p>
