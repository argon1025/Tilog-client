...
export default function UserTopLanguageComponent({ username }) {
    ...
    return(
            ...
            {/* Card */}
            {!toplanguage ? <>fetching...</> 
            : <>
                {Object.keys(toplanguage).map( v=> (
                        <div key={toplanguage[v].size} className="flex flex-col w-full lg:w-32 h-32 border border-gray-200 rounded-3xl bg-gray-800 mr-5">
                        {/* Card icon */}
                        ...
                        <p className="text-sm text-gray-100 font-bold mr-1">
                            ...
                            <p className="text-xs text-gray-400">{toplanguage[v].percent}% Total used</p>
                        </div>
                    </div>
                ))}
            </>
