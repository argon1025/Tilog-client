...
export default function Replies({
  ...
  const RenderRepliesContents = () => {
    // 삭제된 댓글
    if(!!replies.comments_deletedAt) return <span>삭제된 댓글입니다.</span>;
    // 업데이트 모드
  ...
  return (
    <div key={replies.comments_id} className="px-10 ">
      {/* commentTree */}
      <div className="pl-5 border-l-4 pb-10">
        <div className="flex flex-row items-center pt-5">
          <img
            ...
            alt=""
          />
          <span class="ml-2 font-medium text-gray-800" onClick={()=> onClickUserImage(replies.users_userName)}>
        ...
        {!isUpdateMode ? <RenderRepliesContents /> : 
        <div className="flex bg-gray-100 rounded-lg w-full h-28 mt-10">
          <input
            className="px-4 bg-gray-100 w-full h-full rounded-l-lg text-base text-gray-500 px-2 focus:outline-none focus:ring transition text-gray-600 hover:bg-gray-50 active:bg-gray-100 focus:ring-gray-300"
            type="text"
