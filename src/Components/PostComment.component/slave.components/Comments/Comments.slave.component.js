...
export default function Comments({
  ...
  const RednerCommentContents = () => {
    // 삭제된 댓글
    if(!!comment.comments_deletedAt) return <span>삭제된 댓글입니다.</span>;
    // 최종 렌더링
    ...
    if(isRepliesOpened) {
      return (
        <>
          ...
          <InputReply createReply={createReply} commentId={comment.comments_id} />
        </>
      )
    }
  return <></>;
  ...
  const RednerRepliesOpened = () => {
    ...
    // 닫혀있고 답글이 없을 경우
    if(comment.childcount > 0 && !isRepliesOpened) return (
      <button
      className="ml-auto"
      onClick={async() => {
        if(!replies) await getReply(comment.comments_id);
  ...
  return (
    <div className="flex flex-col my-3">
      <div className="flex flex-row items-center">
        ...
        />
        {/** 댓글 작성자 이름 */}
        <span class="ml-2 font-medium text-gray-800" onClick={()=> onClickUserImage(comment.users_userName)}>
      ...
      {/* 업데이트 모드 */}
      {!isUpdateMode ?<RednerCommentContents /> :
