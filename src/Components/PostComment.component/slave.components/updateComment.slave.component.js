...
export default class UpdateCommentComponent extends Component {
    state = {
      commentData: ""
    }
    getOnClick = () =>{
      this.props.updatecomment(this.props.commentid, this.state.commentData);
      this.props.cancel();
    }
    render(){
        return(
            <>
              ...
              <div className="flex bg-gray-100 rounded-lg w-full h-28 mt-10">
                <input
                  ...
                  type="text"
                  placeholder={this.props.htmlcontent}
                  onChange={(event)=>{
                    this.setState({
