...
export function useCommentTools(postsId) {
  ...
  // 새로운 코멘트 작성
  const fetchWriteComment = useCallback(
    async (htmlContent, toast) => {
      try {
        ...
        if (!error.response) {
          ...
          } else if (!error.response.data.message.kr)
  ...
  // 코멘트 수정
  const fetchUpdateComment = useCallback(
    async (commentId, htmlContent, toast) => {
      try {
  ...
  // 코멘트 삭제
  const fetchDeleteComment = useCallback(
    async (commentId, toast) => {
      try {
  ...
  // 코멘트 복구
  const fetchRestoreComment = useCallback(
    async (commentId, toast) => {
      try {
